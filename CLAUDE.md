...
Guidance for AI agents working in this repo. Tables use native Markdown so they render in repository previews. Read the linked `docs/` only when a task needs that depth.
...
When entering WSL from a non-Linux shell, Windows may pass `TEMP`/`TMP` values under
`/mnt/c/...` into the Linux process. Firebase Functions emulators cannot create Unix sockets there.
Before emulator commands, force a Linux temp directory inside the bridged shell:
...
- Tabular data in docs/this file → native Markdown tables. Verify HTML with `tidy`/`xmllint`.
